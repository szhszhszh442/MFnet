...
FOLDER = "/data/shizhihao/csfa/data/" # Replace with your "/path/to/the/ISPRS/dataset/folder/"
BATCH_SIZE = 4
# BATCH_SIZE = 4 # For backbone ViT6-Huge
...
if DATASET == 'Vaihingen':
    ...
    # test_ids = ['5', '21', '15', '30']
    train_ids = ['1', '3', '5', '7', '11', '13', '15', '17', '23', '26', '28', '30', '32', '34', '37']
    test_ids = ['2', '4', '6', '8', '10', '12', '14', '16', '20', '22', '24', '27', '29', '31', '33', '35', '38']
    Stride_Size = 256
    epochs = 80
    save_epoch = 5
    MAIN_FOLDER = FOLDER + 'Vaihingen/'
    DATA_FOLDER = MAIN_FOLDER + 'train/images/top_mosaic_09cm_area{}_*.tif'
    DSM_FOLDER = MAIN_FOLDER + 'train/dsms/top_mosaic_09cm_area{}_*.tif'
    LABEL_FOLDER = MAIN_FOLDER + 'train/masks/top_mosaic_09cm_area{}_*.png'
    TEST_DATA_FOLDER = MAIN_FOLDER + 'test/images/top_mosaic_09cm_area{}_*.tif'
    TEST_DSM_FOLDER = MAIN_FOLDER + 'test/dsms/top_mosaic_09cm_area{}_*.tif'
    TEST_LABEL_FOLDER = MAIN_FOLDER + 'test/masks/top_mosaic_09cm_area{}_*.png'
...
class ISPRS_dataset(torch.utils.data.Dataset):
    def __init__(self, ids, data_files=DATA_FOLDER, label_files=LABEL_FOLDER,
        ...
        if DATASET == 'Vaihingen' and IS_PREPROCESSED:
            self.data_files = []
            self.dsm_files = []
            self.label_files = []
            for id in ids:
                data_pattern = DATA_FOLDER.format(id)
                dsm_pattern = DSM_FOLDER.format(id)
                label_pattern = LABEL_FOLDER.format(id)
                
                data_matches = sorted(glob(data_pattern))
                dsm_matches = sorted(glob(dsm_pattern))
                label_matches = sorted(glob(label_pattern))
                
                self.data_files.extend(data_matches)
                self.dsm_files.extend(dsm_matches)
                self.label_files.extend(label_matches)
        else:
        ...
        if len(self.data_files) == 0:
            raise KeyError('No data files found for ids: {}'.format(ids))
    ...
    def __getitem__(self, i):
        ...
        if random_idx in self.data_cache_.keys():
            ...
            if DATASET == 'Potsdam':
                ...
                data = np.array(Image.open(self.data_files[random_idx])).transpose((2, 0, 1))
                data = 1 / 255 * np.asarray(data, dtype='float32')
            else:
            ## Vaihingen IRRG
        ...
        if random_idx in self.dsm_cache_.keys():
            ...
            if DATASET == 'Vaihingen' and IS_PREPROCESSED:
                dsm = np.array(Image.open(self.dsm_files[random_idx]), dtype='float32')
            else:
        ...
        if random_idx in self.label_cache_.keys():
            ...
            if DATASET == 'Hunan':
                ...
                label = np.array(Image.open(self.label_files[random_idx]), dtype='int64')
                label[label == 6] = 5
            else:
...
def loss_calc(pred, label, weights=None):
    """
