...
class UNetFormer(nn.Module):
    def __init__(self,
        ...
        # MaskDecoder输出通道数（multimask_output=False时为1，True时为3）
        self.num_mask_outputs = 1
        
        self.mask_conv = nn.Sequential(
            nn.Conv2d(self.num_mask_outputs, decode_channels, kernel_size=3, padding=1),
    ...
    def forward(self, x, y, mode='Train',
        ...
        # 使用可微分SAM提示生成模块
        # 如果外部提供了提示，使用外部的；否则自动生成
        if point_coords is None and boxes is None and masks is None:
            # 自动生成提示
        ...
        mask_output, iou_pred = self.mask_decoder(
            image_embeddings=res1,
            image_pe=image_pe,
        ...
        # mask_output: (B, num_masks, H, W)
        # 处理所有mask通道
        B, num_masks, H_mask, W_mask = mask_output.shape
        
        # 将所有mask输入到卷积层
        # (B, num_masks, H, W) -> (B*num_masks, 1, H, W)
        mask_features = mask_output.view(B * num_masks, 1, H_mask, W_mask)
        
        # 对齐到res1的尺寸
        if mask_features.shape[2:] != res1.shape[2:]:
        ...
        # 卷积处理
        mask_conv_out = self.mask_conv(mask_features)  # (B*num_masks, decode_channels, H', W')
        
        # 如果有多个mask，取平均或加权平均
        if num_masks > 1:
            # (B*num_masks, decode_channels, H', W') -> (B, num_masks, decode_channels, H', W')
            mask_conv_out = mask_conv_out.view(B, num_masks, -1, mask_conv_out.shape[2], mask_conv_out.shape[3])
            # 使用IoU预测作为权重进行加权平均
            iou_weights = F.softmax(iou_pred, dim=1)  # (B, num_masks)
            mask_conv_out = torch.einsum('bn,bnchw->bchw', iou_weights, mask_conv_out)
        else:
            # (B*1, decode_channels, H', W') -> (B, decode_channels, H', W')
            mask_conv_out = mask_conv_out.view(B, -1, mask_conv_out.shape[2], mask_conv_out.shape[3])
        
        # 分割头
        seg_output = self.segmentation_head(mask_conv_out)
