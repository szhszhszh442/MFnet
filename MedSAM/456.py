...
class UNetFormer(nn.Module):
    def __init__(self,
        ...
        self.fusion1 = DynamicConvFusion(encoder_channels[0])
        self.fusion2 = DynamicConvFusion(encoder_channels[1])
        self.fusion3 = DynamicConvFusion(encoder_channels[2])
        self.fusion4 = DynamicConvFusion(encoder_channels[3])
    ...
    def forward(self, x, y, mode='Train',
        ...
        if point_coords is not None or boxes is not None or masks is not None:
            ...
            if point_coords_gen is not None or masks_gen is not None:
                ...
                # point_coords_mapped = point_coords_mapped * (self.image_encoder.img_size / max(h_now, w_now))
                # print(point_coords_mapped)
