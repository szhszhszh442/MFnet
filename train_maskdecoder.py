...
from MedSAM.UNetFormer_SAM_queryhead import UNetFormer as MFNet
from MedSAM.losses import MultiTaskLoss, CombinedLoss
from MedSAM.learnable_loss import LearnableMultiTaskLoss, AdaptiveWeightLoss
try:
...
def train(net, optimizer, epochs, scheduler=None, weights=WEIGHTS, save_epoch=1, 
          use_multi_task_loss=True, use_learnable_weights=True):
    ...
    if use_multi_task_loss:
        if use_learnable_weights:
            criterion = LearnableMultiTaskLoss(
                num_classes=N_CLASSES,
                initial_weights=[1.0, 1.0, 0.5, 0.5, 0.5]
            ).cuda()
            print("Using Learnable Multi-Task Loss (weights will be learned automatically)")
            ...
            criterion = CombinedLoss(N_CLASSES, weights=weights, loss_configs=loss_configs).cuda()
            print("Using Fixed-Weight Multi-Task Loss (CE + Dice + Focal + Tversky + Boundary)")
    else:
        criterion = None
    ...
    for e in range(1, epochs + 1):
        ...
        for batch_idx, (data, dsm, target) in enumerate(train_loader):
            ...
            if use_multi_task_loss and criterion is not None:
            ...
            if iter_ % 100 == 0:
                ...
                if use_multi_task_loss and criterion is not None:
                    loss_str = f"Loss: {loss.data:.6f}\t"
                    
                    if use_learnable_weights:
                        loss_str += f"Dice: {loss_dict['dice_loss'].data:.4f}\t"
                        loss_str += f"Tversky: {loss_dict['tversky_loss'].data:.4f}\t"
                        loss_str += f"Focal: {loss_dict['focal_loss'].data:.4f}\t"
                        loss_str += f"Boundary: {loss_dict['boundary_loss'].data:.4f}\n"
                        loss_str += f"Weights: CE={loss_dict['weight_ce'].data:.3f}\t"
                        loss_str += f"Dice={loss_dict['weight_dice'].data:.3f}\t"
                        loss_str += f"Focal={loss_dict['weight_focal'].data:.3f}\t"
                        loss_str += f"Tversky={loss_dict['weight_tversky'].data:.3f}\t"
                        loss_str += f"Boundary={loss_dict['weight_boundary'].data:.3f}"
                    else:
                        loss_str += f"Dice: {loss_dict['dice_loss'].data:.4f}\t"
                        loss_str += f"Tversky: {loss_dict['tversky_loss'].data:.4f}\t"
                        loss_str += f"Focal: {loss_dict['focal_loss'].data:.4f}\t"
                ...
                print('Train (epoch {}/{}) [{}/{} ({:.0f}%)]\t{}\tAccuracy: {}'.format(
                    e, epochs, batch_idx, len(train_loader),
                    100. * batch_idx / len(train_loader), loss_str, accuracy(pred, gt)))
            iter_ += 1
        ...
        if e % save_epoch == 0:
            ...
            if MIoU > MIoU_best:
                if DATASET == 'Vaihingen':
                    ...
                    torch.save(net.state_dict(), './resultsv/{}_fusiongdquery_epoch{}_{}'.format(MODEL, e, MIoU))
